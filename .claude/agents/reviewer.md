...
- **Conventions:** `PYTHONPATH=.:services:agent`; `from shared.*` imports; fixtures/recipe respected; no raw `settings.get()` in the typed-config era.
...
`PYTHONPATH=.:services:agent ./.venv/bin/python -m pytest -q agent/tests/<...>_test.py`.
