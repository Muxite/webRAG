...
**Agent Worker** (`interface_agent.py`): Consumes RabbitMQ tasks, initializes and reuses connectors via dependency injection, verifies dependencies ready before consuming, runs Agent per task, updates status in Redis, maintains presence, handles graceful shutdown
...
**Gateway Service** (`gateway_service.py`, `api.py`): FastAPI with Supabase auth, Redis task storage, RabbitMQ publishing, per-user quota enforcement, worker availability tracking
...
Environment variables configured via `services/.env` and `services/keys.env`.
