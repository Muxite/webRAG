...
def create_app(service: Optional[GatewayService] = None) -> FastAPI:
    ...
    if allowed_keys:
        special_keys = key_provider._load_special_keys()
        if special_keys:
            app.state.api_key = next(iter(special_keys))
        else:
            test_key = key_provider.get_expected_key()
            app.state.api_key = test_key if test_key else next(iter(allowed_keys))
    else:
