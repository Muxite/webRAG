...
* **readiness** — the step produced no usable payload at all (a visit with no content, a
  ...
  (i.e. LINKS), which answers "what does this node provide downstream", not "did this
...
is expected to fall back to whatever it did before. Only a *positive* absence — a missing
payload, a missing datum, a missing subject — reports ``satisfied=False``.
...
# for cues whose datum has no stable page wording (a year, a count, a price).
...
def _subject_tokens(text: str) -> List[str]:
    ...
    capped, because the long tokens carry the identity ("quesnel") and the short head noun
    ("lake") is shared with every wrong page. Empty when the text names no proper noun — the
    subject requirement then simply does not apply.
    """
    tokens: List[str] = []
    seen = set()
...
def _number_near(evidence_low: str, variants: Sequence[str]) -> bool:
    """A number within ``_NUMBER_WINDOW`` chars of the datum's wording.

    An empty ``variants`` means the datum has no stable page wording (a year, a count),
    so any number in the evidence counts.
    """
    if not variants:
        return bool(_DIGIT_RE.search(evidence_low))
    for variant in variants:
        for match in re.finditer(re.escape(variant), evidence_low):
            start = max(0, match.start() - _NUMBER_WINDOW)
            if _DIGIT_RE.search(evidence_low[start : match.end() + _NUMBER_WINDOW]):
                return True
    return False
...
def evaluate_step_contract(node: Any, mandate: str = "") -> ContractSatisfaction:
    ...
    ``applicable=False`` means "no verdict" — no checkable requirement could be derived
    (an action that retrieves nothing, or a contract text with neither a measurable datum
    nor an identity token). Callers treat that as "fall back to the previous signal".
    """
    details = getattr(node, "details", {}) or {}
