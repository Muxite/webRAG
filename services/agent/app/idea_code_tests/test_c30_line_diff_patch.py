        ...
        "leaves": [
            {
                ...
                "instruction": (
                    ...
                    "file — use run_python to sanity-check: build several old/new pairs "
                    "(identical lists, completely disjoint lists, a list with a real change "
                    "in the middle, an empty list on one side), confirm "
                    "apply_patch(old, diff_lines(old, new)) == new for each, and confirm the "
                    "count of 'equal' ops matches how many lines you'd expect to be common by "
                    "inspection (e.g. for old=['a','b','c','d','e'], "
                    "new=['a','x','c','d','f'] the common lines are a, c, d -- 3 'equal' "
                    "ops). Also confirm apply_patch raises ValueError when given an "
                    "incomplete op list that doesn't fully consume old. Fix issues with "
                    "patch_file/run_python until confident, then finish."
                ),
