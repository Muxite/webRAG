...
@pytest.mark.asyncio
async def test_agent_only_writes_to_redis(monkeypatch, caplog):
    """
    Architecture test: Verify agents only write to Redis, never to Supabase.
    This enforces worker isolation - agents should not have Supabase access.
    """
    caplog.set_level("INFO")
    correlation_id = str(uuid.uuid4())
    input_q = f"test.agent.mandates.{correlation_id}"
    ...
    mandate = "Say 'redis-only' and exit immediately. No search, no visit."
    task = {"mandate": mandate, "max_ticks": 2, "correlation_id": correlation_id}
    await rmq.publish_task(correlation_id=correlation_id, payload=task)
    ...
    assert len(statuses) >= 3, "Agent should update status in Redis"
    assert statuses[0] == "accepted", "Agent writes to Redis"
    assert "completed" in statuses, "Agent completes and writes to Redis"
    
    assert "supabase" not in caplog.text.lower(), "Agent should not interact with Supabase"
    assert "Supabase" not in caplog.text, "Agent should not interact with Supabase"