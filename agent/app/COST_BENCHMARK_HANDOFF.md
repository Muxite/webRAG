...
- `testing/scaffold_compiler.py` (NEW) — `compile_plan(mandate, author_model, agent_io)`,
  **cache-first** by `mandate_hash`; strong meta-prompt → DAG JSON; disk-cache at
  `agent/compiled_plans/<hash>.json` (the "paid-offline" artifact, env
...
`graph_compiled`. `PYTHONPATH=.:services:agent ./.venv/bin/python -m pytest -q
agent/tests/{compiled_plan,scaffold_compiler,execution_compiled,breadth_argmax_validators,mixed_dag_validators}_test.py`.
...
Artifact: `agent/idea_test_results/recovery_curve.png`.
...
PYTHONPATH=.:services:agent ./.venv/bin/python -m agent.app.idea_test_runner
...
- `idea_graph_analyzer.py` has a stray `from app.…` import → benchmark runner needs `PYTHONPATH=.:services:agent`.
- Artifacts from this session live in `agent/idea_test_results/` (`recovery_curve.{csv,png}`, `web_fixtures/`, per-run JSON).
