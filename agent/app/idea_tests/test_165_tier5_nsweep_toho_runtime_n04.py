"""
Test 165: N-SWEEP member N=4 -- roster discovery -> 4-way independent fan-out -> argmin
Level: graph   Weight: long   Difficulty: 6/10   Category: N-sweep Breadth Aggregation
...
every member of the family and per-item difficulty cannot drift with N.
  ...
  ROSTER (discovered, one page)  'List of Godzilla films' numbers the Japanese (Toho) films in
    release order. The mandate names only that page and the prefix length 4; no film title
    appears in it, so the roster must be fetched before any fan-out can begin.
  FAN-OUT (4 independent one-page reads)  each film's own article -> infobox running time.
  MERGE  argmin over the 4 runtimes, plus the combined total (wrong if one item is missing).
...
provenance, margins and the Wikidata cross-check that it FAILED are in the family module):
  ...
  shortest of the first 4: Godzilla Raids Again -- 81 minutes   <-- KEYSTONE
  runner-up: 88 minutes  ->  margin 7 minutes
  combined total running time of the 4 films: 362 minutes

Decision-critical item: #2 (the argmin). Dropping it flips the answer; every other item is
argmin-irrelevant but still moves the total. Sources: https://en.wikipedia.org/wiki/List_of_Godzilla_films plus the
4 linked film articles.
"""
...
def get_test_metadata() -> Dict[str, Any]:
    ...
    return {
        "test_id": "165",
        "test_name": "N-sweep N=4: roster discovery + 4-way fan-out -> shortest running time",
...
def get_task_statement() -> str:
    """Return the mandate (family wording, N=4).
...
def get_compiled_plan() -> Dict[str, Any]:
    """Return the offline-authored DAG (schema v2): one roster leaf + 4 positional film
