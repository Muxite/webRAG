"""
Test 216: Tier 5: Derived composed-unit ratio (rail journey average speed)
...
operands live on the SAME Wikipedia page:
  A. Tōkaidō Shinkansen (Japan) -- read the route (line) length between Tokyo and Shin-Osaka, in km.
  B. Tōkaidō Shinkansen (Japan) -- read the FASTEST scheduled Nozomi journey time between Tokyo and Shin-Osaka, in hours and minutes.
The KEYSTONE is the RATIO: Tōkaidō Shinkansen (Japan) / Tōkaidō Shinkansen (Japan) = 219.32 km/h. This number is PRINTED
...
Ground truth (verified against live English Wikipedia via WebFetch, 2026-08-31):
  Tōkaidō Shinkansen (Japan): 515.4 (https://en.wikipedia.org/wiki/T%C5%8Dkaid%C5%8D_Shinkansen)
  Tōkaidō Shinkansen (Japan): 2.35 (https://en.wikipedia.org/wiki/T%C5%8Dkaid%C5%8D_Shinkansen)
  Line length Tokyo-Shin-Osaka 515.4 km; fastest Nozomi journey time 2h21m (=2.35 h). Neither page divides these into an average speed (the page states only the train's max operating speed, a different figure).
  RATIO = 219.32 km/h

Margin (near-miss protection; every plausible wrong computation is decisively separated from the
+/-3% acceptance band [212.7404, 225.8996] around 219.32):
  * dropping the 21 minutes and dividing by 2 hours flat: 257.7 -- 17% away from the correct value, decisively outside the acceptance band
  * the train's max/operating speed printed on the page (285-320 km/h): 300 -- 37% away from the correct value, decisively outside the acceptance band

Anti-leak: the ratio value 219.32 km/h appears in no source page and is
...
OP_A: Dict[str, Any] = {
    "key": 'distance', "label": 'Tōkaidō Shinkansen (Japan)', "value": 515.4, "url": 'https://en.wikipedia.org/wiki/T%C5%8Dkaid%C5%8D_Shinkansen',
    "fact": 'the route (line) length between Tokyo and Shin-Osaka, in km', "slug_rx": 'wiki/t\\S{0,8}kaid\\S{0,8}_shinkansen', "name_rx": 't\\S{0,8}kaid\\S{0,8}\\s+shinkansen',
}
OP_B: Dict[str, Any] = {
    "key": 'time', "label": 'Tōkaidō Shinkansen (Japan)', "value": 2.35, "url": 'https://en.wikipedia.org/wiki/T%C5%8Dkaid%C5%8D_Shinkansen',
    "fact": 'the FASTEST scheduled Nozomi journey time between Tokyo and Shin-Osaka, in hours and minutes', "slug_rx": 'wiki/t\\S{0,8}kaid\\S{0,8}_shinkansen', "name_rx": 't\\S{0,8}kaid\\S{0,8}\\s+shinkansen',
}
...
_DECOYS = [('dropping the 21 minutes and dividing by 2 hours flat', 257.7), ("the train's max/operating speed printed on the page (285-320 km/h)", 300.0)]
...
def get_test_metadata() -> Dict[str, Any]:
    return {
        "test_id": '216',
        "test_name": 'Tier 5: Derived composed-unit ratio (rail journey average speed)',
...
def get_task_statement() -> str:
    return (
        ...
        "yourself.\n\n"
        "Report (a) the computed average speed (distance / time) (a single number, in the correct unit -- the "
...
def get_required_deliverables() -> List[str]:
    return [
        "The computed AVERAGE SPEED (distance / time) value (the keystone)",
        "Tōkaidō Shinkansen (Japan)'s raw value (with unit)",
        "Tōkaidō Shinkansen (Japan)'s raw value (with unit)",
...
def validate_keystone(result: Dict[str, Any], observability: Dict[str, Any]) -> Dict[str, Any]:
    """KEYSTONE (hard 0/1): the computed RATIO (219.32 km/h) is present, grounded."""
    passed = _keystone_ok(result, observability)
    return {"check": "keystone_216", "passed": passed, "score": 1.0 if passed else 0.0,
...
def validate_composed_unit(result: Dict[str, Any], observability: Dict[str, Any]) -> Dict[str, Any]:
    """GATED secondary: the answer is a COMPOSED num/den unit (km/h) -- checks both the
    ...
    text = _all_text(result).lower()
    num_ok = bool(re.search('km', text))
    den_ok = bool(re.search('(?:h\\b|hour|hr)', text))
