"""
Test 213: Tier 5: Derived difference over same-unit operands (national park area gap)
...
operands live on TWO DIFFERENT Wikipedia pages:
  A. Wood Buffalo National Park (Canada) -- read its area, in km2.
  B. Kruger National Park (South Africa) -- read its area, in km2.
The KEYSTONE is the ABSOLUTE DIFFERENCE: |Wood Buffalo National Park (Canada) - Kruger National Park (South Africa)| = 25,118 km2. This number is PRINTED
...
Ground truth (verified against live English Wikipedia via WebFetch, 2026-08-31):
  Wood Buffalo National Park (Canada): 44,741 (https://en.wikipedia.org/wiki/Wood_Buffalo_National_Park)
  Kruger National Park (South Africa): 19,623 (https://en.wikipedia.org/wiki/Kruger_National_Park)
  Wood Buffalo National Park area 44,741 km2 (Canada's largest national park); Kruger National Park area 19,623 km2. Neither page states the gap.
  ABSOLUTE DIFFERENCE = 25,118 km2

Margin (near-miss protection; every plausible wrong computation is decisively separated from the
+/-1% acceptance band [24,866.82, 25,369.18] around 25,118):
  * sum instead of difference: 64,364 -- 156% away from the correct value, decisively outside the acceptance band

Anti-leak: the absolute difference value 25,118 km2 appears in no source page and is
...
OP_A: Dict[str, Any] = {
    "key": 'woodbuffalo', "label": 'Wood Buffalo National Park (Canada)', "value": 44741.0, "url": 'https://en.wikipedia.org/wiki/Wood_Buffalo_National_Park',
    "fact": 'its area, in km2', "slug_rx": 'wiki/wood_buffalo_national_park', "name_rx": 'wood\\s+buffalo',
}
OP_B: Dict[str, Any] = {
    "key": 'kruger', "label": 'Kruger National Park (South Africa)', "value": 19623.0, "url": 'https://en.wikipedia.org/wiki/Kruger_National_Park',
    "fact": 'its area, in km2', "slug_rx": 'wiki/kruger_national_park', "name_rx": '\\bkruger\\b',
}
...
_DECOYS = [('sum instead of difference', 64364.0)]
...
def get_test_metadata() -> Dict[str, Any]:
    return {
        "test_id": '213',
        "test_name": 'Tier 5: Derived difference over same-unit operands (national park area gap)',
...
def get_task_statement() -> str:
    return (
        ...
        + 'Then COMPUTE the ABSOLUTE DIFFERENCE between the two values you read (the larger minus the smaller), in km2.' + "\n"
...
def get_required_deliverables() -> List[str]:
    return [
        ...
        "Wood Buffalo National Park (Canada)'s raw value (with unit)",
        "Kruger National Park (South Africa)'s raw value (with unit)",
...
def validate_keystone(result: Dict[str, Any], observability: Dict[str, Any]) -> Dict[str, Any]:
    """KEYSTONE (hard 0/1): the computed ABSOLUTE DIFFERENCE (25,118 km2) is present, grounded."""
    passed = _keystone_ok(result, observability)
    return {"check": "keystone_213", "passed": passed, "score": 1.0 if passed else 0.0,
...
def get_compiled_plan() -> Dict[str, Any]:
    ...
    return {
        ...
        "aggregation": (
            ...
            " and one for " + OP_B["label"] + ". " + 'Then COMPUTE the ABSOLUTE DIFFERENCE between the two values you read (the larger minus the smaller), in km2.' +
