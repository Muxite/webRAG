"""
Test 214: Tier 5: Derived dimensionless ratio (hydropower installed-capacity multiple)
...
operands live on TWO DIFFERENT Wikipedia pages:
  A. Xiluodu Dam (China) -- read its total INSTALLED GENERATING CAPACITY, in MW.
  B. Longtan Dam (China) -- read its total INSTALLED GENERATING CAPACITY, in MW.
The KEYSTONE is the RATIO: Xiluodu Dam (China) / Longtan Dam (China) = 2.1569 times. This number is PRINTED
...
Ground truth (verified against live English Wikipedia via WebFetch, 2026-08-31):
  Xiluodu Dam (China): 13,860 (https://en.wikipedia.org/wiki/Xiluodu_Dam)
  Longtan Dam (China): 6,426 (https://en.wikipedia.org/wiki/Longtan_Dam)
  Xiluodu Dam installed capacity 13,860 MW; Longtan Dam installed capacity 6,426 MW. Neither page states the multiple.
  RATIO = 2.1569 times

Margin (near-miss protection; every plausible wrong computation is decisively separated from the
+/-2% acceptance band [2.1138, 2.2] around 2.1569):
  * inverted ratio (Longtan / Xiluodu): 0.4636 -- 79% away from the correct value, decisively outside the acceptance band
  * difference in MW mistaken for a ratio: 7,434 -- 344561% away from the correct value, decisively outside the acceptance band

Anti-leak: the ratio value 2.1569 times appears in no source page and is
...
OP_A: Dict[str, Any] = {
    "key": 'xiluodu', "label": 'Xiluodu Dam (China)', "value": 13860.0, "url": 'https://en.wikipedia.org/wiki/Xiluodu_Dam',
    "fact": 'its total INSTALLED GENERATING CAPACITY, in MW', "slug_rx": 'wiki/xiluodu_dam', "name_rx": 'xiluodu',
}
OP_B: Dict[str, Any] = {
    "key": 'longtan', "label": 'Longtan Dam (China)', "value": 6426.0, "url": 'https://en.wikipedia.org/wiki/Longtan_Dam',
    "fact": 'its total INSTALLED GENERATING CAPACITY, in MW', "slug_rx": 'wiki/longtan_dam', "name_rx": 'longtan',
}
...
_DECOYS = [('inverted ratio (Longtan / Xiluodu)', 0.4636), ('difference in MW mistaken for a ratio', 7434.0)]
...
def get_test_metadata() -> Dict[str, Any]:
    return {
        "test_id": '214',
        "test_name": 'Tier 5: Derived dimensionless ratio (hydropower installed-capacity multiple)',
...
def get_task_statement() -> str:
    return (
        ...
        "yourself.\n\n"
        "Report (a) the computed ratio (xiluodu / longtan) (a single number, in the correct unit -- the "
...
def get_required_deliverables() -> List[str]:
    return [
        "The computed RATIO (Xiluodu / Longtan) value (the keystone)",
        "Xiluodu Dam (China)'s raw value (with unit)",
        "Longtan Dam (China)'s raw value (with unit)",
...
def validate_keystone(result: Dict[str, Any], observability: Dict[str, Any]) -> Dict[str, Any]:
    """KEYSTONE (hard 0/1): the computed RATIO (2.1569 times) is present, grounded."""
    passed = _keystone_ok(result, observability)
    return {"check": "keystone_214", "passed": passed, "score": 1.0 if passed else 0.0,
