"""
Test 210: Tier 5: Derived difference over same-unit operands (chimney height gap)
...
operands live on TWO DIFFERENT Wikipedia pages:
  A. GRES-2 Power Station chimney (Ekibastuz, Kazakhstan) -- read the height of its flue-gas chimney/stack, in meters.
  B. Inco Superstack (Sudbury, Ontario, Canada) -- read its height, in meters.
The KEYSTONE is the ABSOLUTE DIFFERENCE: |GRES-2 Power Station chimney (Ekibastuz, Kazakhstan) - Inco Superstack (Sudbury, Ontario, Canada)| = 38.7 m. This number is PRINTED
...
Ground truth (verified against live English Wikipedia via WebFetch, 2026-08-31):
  GRES-2 Power Station chimney (Ekibastuz, Kazakhstan): 419.7 (https://en.wikipedia.org/wiki/GRES-2_Power_Station)
  Inco Superstack (Sudbury, Ontario, Canada): 381 (https://en.wikipedia.org/wiki/Inco_Superstack)
  GRES-2 flue-gas stack 419.7 m (1,377 ft) is stated as the world's tallest chimney; Inco Superstack 381 m (1,250 ft). Neither page states the gap.
  ABSOLUTE DIFFERENCE = 38.7 m

Margin (near-miss protection; every plausible wrong computation is decisively separated from the
+/-2% acceptance band [37.926, 39.474] around 38.7):
  * sum instead of difference: 800.7 -- 1969% away from the correct value, decisively outside the acceptance band
  * feet-based difference (1,377-1,250 ft): 127 -- 228% away from the correct value, decisively outside the acceptance band

Anti-leak: the absolute difference value 38.7 m appears in no source page and is
...
OP_A: Dict[str, Any] = {
    "key": 'gres2', "label": 'GRES-2 Power Station chimney (Ekibastuz, Kazakhstan)', "value": 419.7, "url": 'https://en.wikipedia.org/wiki/GRES-2_Power_Station',
    "fact": 'the height of its flue-gas chimney/stack, in meters', "slug_rx": 'wiki/gres-?2_power_station', "name_rx": 'gres-?2|ekibastuz',
}
OP_B: Dict[str, Any] = {
    "key": 'inco', "label": 'Inco Superstack (Sudbury, Ontario, Canada)', "value": 381.0, "url": 'https://en.wikipedia.org/wiki/Inco_Superstack',
    "fact": 'its height, in meters', "slug_rx": 'wiki/inco_superstack', "name_rx": 'inco\\s+superstack|superstack',
}
...
_DECOYS = [('sum instead of difference', 800.7), ('feet-based difference (1,377-1,250 ft)', 127.0)]
...
def get_test_metadata() -> Dict[str, Any]:
    return {
        "test_id": '210',
        "test_name": 'Tier 5: Derived difference over same-unit operands (chimney height gap)',
...
def get_task_statement() -> str:
    return (
        ...
        + 'Then COMPUTE the ABSOLUTE DIFFERENCE between the two values you read (the larger minus the smaller), in m.' + "\n"
...
def get_required_deliverables() -> List[str]:
    return [
        ...
        "GRES-2 Power Station chimney (Ekibastuz, Kazakhstan)'s raw value (with unit)",
        "Inco Superstack (Sudbury, Ontario, Canada)'s raw value (with unit)",
...
def validate_keystone(result: Dict[str, Any], observability: Dict[str, Any]) -> Dict[str, Any]:
    """KEYSTONE (hard 0/1): the computed ABSOLUTE DIFFERENCE (38.7 m) is present, grounded."""
    passed = _keystone_ok(result, observability)
    return {"check": "keystone_210", "passed": passed, "score": 1.0 if passed else 0.0,
...
def get_compiled_plan() -> Dict[str, Any]:
    ...
    return {
        ...
        "aggregation": (
            ...
            " and one for " + OP_B["label"] + ". " + 'Then COMPUTE the ABSOLUTE DIFFERENCE between the two values you read (the larger minus the smaller), in m.' +
