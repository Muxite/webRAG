...
_CHAIN_WAYPOINTS = [
    {"name": "aviator (Earhart)", "name_rx": HOP_AVIATOR, "slug_rx": CITE_AVIATOR},
    {"name": "town (Atchison)", "name_rx": HOP_TOWN, "slug_rx": CITE_TOWN},
]
...
def _keystone_ok(result: Dict[str, Any], observability: Dict[str, Any] = None) -> bool:
    """Grounding requirement: a correct-but-ungrounded parametric-memory guess (zero page visits)
    must not earn keystone credit."""
    grounded = int((observability or {}).get("visit", {}).get("count", 0) or 0) > 0
    return grounded and bool(re.search(KEYSTONE_ELEVATION, _primary_text(result)))
...
def validate_chain_coverage(result: Dict[str, Any], observability: Dict[str, Any]) -> Dict[str, Any]:
    ...
    crediting the aviator/town names on text presence alone regardless of whether either page was
