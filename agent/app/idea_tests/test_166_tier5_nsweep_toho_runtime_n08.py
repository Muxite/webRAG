"""
Test 166: N-SWEEP member N=8 -- roster discovery -> 8-way independent fan-out -> argmin
Level: graph   Weight: long   Difficulty: 7/10   Category: N-sweep Breadth Aggregation
...
every member of the family and per-item difficulty cannot drift with N.
  ...
  ROSTER (discovered, one page)  'List of Godzilla films' numbers the Japanese (Toho) films in
    release order. The mandate names only that page and the prefix length 8; no film title
    appears in it, so the roster must be fetched before any fan-out can begin.
  FAN-OUT (8 independent one-page reads)  each film's own article -> infobox running time.
  MERGE  argmin over the 8 runtimes, plus the combined total (wrong if one item is missing).
...
provenance, margins and the Wikidata cross-check that it FAILED are in the family module):
  ...
  shortest of the first 8: Godzilla Raids Again -- 81 minutes   <-- KEYSTONE
  runner-up: 86 minutes  ->  margin 5 minutes
  combined total running time of the 8 films: 722 minutes
...
def get_test_metadata() -> Dict[str, Any]:
    ...
    return {
        "test_id": "166",
        "test_name": "N-sweep N=8: roster discovery + 8-way fan-out -> shortest running time",
...
def get_task_statement() -> str:
    """Return the mandate (family wording, N=8).
...
def get_compiled_plan() -> Dict[str, Any]:
    """Return the offline-authored DAG (schema v2): one roster leaf + 8 positional film
