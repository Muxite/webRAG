"""
Test 168: N-SWEEP member N=32 -- roster discovery -> 32-way independent fan-out -> argmin
Level: graph   Weight: long   Difficulty: 9/10   Category: N-sweep Breadth Aggregation
...
every member of the family and per-item difficulty cannot drift with N.
  ...
  ROSTER (discovered, one page)  'List of Godzilla films' numbers the Japanese (Toho) films in
    release order. The mandate names only that page and the prefix length 32; no film title
    appears in it, so the roster must be fetched before any fan-out can begin.
  FAN-OUT (32 independent one-page reads)  each film's own article -> infobox running time.
  MERGE  argmin over the 32 runtimes, plus the combined total (wrong if one item is missing).
...
provenance, margins and the Wikidata cross-check that it FAILED are in the family module):
  ...
  shortest of the first 32: All Monsters Attack -- 70 minutes   <-- KEYSTONE
  runner-up: 81 minutes  ->  margin 11 minutes
  combined total running time of the 32 films: 3051 minutes
...
def get_test_metadata() -> Dict[str, Any]:
    ...
    return {
        "test_id": "168",
        "test_name": "N-sweep N=32: roster discovery + 32-way fan-out -> shortest running time",
...
def get_task_statement() -> str:
    """Return the mandate (family wording, N=32).
...
def get_compiled_plan() -> Dict[str, Any]:
    """Return the offline-authored DAG (schema v2): one roster leaf + 32 positional film
