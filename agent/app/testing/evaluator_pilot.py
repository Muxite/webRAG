...
``agent/idea_test_results/`` carries ``validation.grep_validations``: an ordered list
...
* The ``e-valuator`` PyPI package (deps: numpy/pandas/scikit-learn/scipy) is deliberately NOT
  added to ``agent/requirements.txt`` — mirror the ConSol precedent. Run this pilot in
