...
#: `content` on 2103/2103 recorded visit results) or that carry link bookkeeping the judge cannot
#: score. Dropped only when the blob is over budget, so an under-budget candidate is unaffected.
#: `_compact_details_for_expansion` drops the same duplicates for the planner prompt.
...
def _serialize_details_for_prompt(details: Dict[str, Any], max_chars: int) -> str:
    ...
    candidates that hit the cap reached the judge as an unterminated blob, and every field
    serialized after the cut was gone with it: `visit_url` and `visit_content_length` on
    ...
    the output is byte-identical to `_safe_serialize_details`, so the 37% of candidates that fit
    are unaffected.
    """
...
class LlmEvaluationPolicy(EvaluationPolicy):
    ...
    def _build_messages(self, graph: IdeaDag, node: IdeaNode) -> List[Dict[str, str]]:
        ...
        for entry in path:
            details_text = _serialize_details_for_prompt(entry.details, max_detail_chars)
            serialized.append(
                {
