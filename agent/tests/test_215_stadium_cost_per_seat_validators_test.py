"""
Offline unit tests for Tier 5: Derived composed-unit ratio (stadium construction cost per seat) (test 215) -- free.

Covers the keystone gate (grounded computed ratio), in single- and multi-line
...
guess must still score 0); the composed num/den unit secondary; and that the compiled plan is a well-formed
2-leaf pure-fan-out DAG that leaks neither operand value nor the derived value (the
statement-leak test).
"""
from agent.app.idea_tests import test_215_tier5_stadium_cost_per_seat as t
...
_FULL_SINGLE = (
    "Puskás Aréna (Budapest, Hungary): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
    "Puskás Aréna (Budapest, Hungary): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
    "Computed ratio = " + _fmt(t.DERIVED) + " " + t.DERIVED_UNIT + "."
)
...
def test_composed_unit_secondary():
    r = _r(_FULL_SINGLE)
    assert t.validate_composed_unit(r, _OBS)["score"] == 1.0
...
def test_deliverables_list_primary_slot_drives_keystone():
    r = {
        "deliverables": [
            ...
            "Puskás Aréna (Budapest, Hungary): " + _fmt(_A_VAL) + "; Puskás Aréna (Budapest, Hungary): " + _fmt(_B_VAL) + ".",
...
def test_wrong_decoy_value_gates_credit_bearing_checks_to_zero():
    # The engineered near-miss decoy (using the rugby-configuration capacity (35,169) instead = 15157.0) reported instead of the
    ...
    wrong = (
        "Puskás Aréna (Budapest, Hungary): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Puskás Aréna (Budapest, Hungary): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "Computed value = " + _fmt(15157.0) + " " + t.DERIVED_UNIT + " (a miscalculation)."
    )
    r = _r(wrong)
    ...
    assert t.validate_composed_unit(r, _OBS)["score"] == 0.0   # gated on keystone


def test_composed_unit_missing_unit_zeroes_secondary():
    # Correct keystone value present, but reported as a bare number with no unit words at all.
    text = (
        "Puskás Aréna (Budapest, Hungary): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Puskás Aréna (Budapest, Hungary): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "The answer is " + _fmt(t.DERIVED) + "."
    )
    r = _r(text)
