...
def test_ungrounded_correct_value_gates_to_zero():
    ...
    assert t.validate_keystone_pipes(r, ungrounded_obs)["score"] == 0.0
    assert t.validate_keystone_pipes(r, ungrounded_obs)["passed"] is False
    assert t.validate_survivor_and_chain(r, ungrounded_obs)["score"] == 0.0
    assert t.validate_citations(r, ungrounded_obs)["score"] == 0.0
    scores = [
        t.validate_visits(r, ungrounded_obs)["score"],
        t.validate_keystone_pipes(r, ungrounded_obs)["score"],
        t.validate_branch_exploration(r, ungrounded_obs)["score"],
        t.validate_survivor_and_chain(r, ungrounded_obs)["score"],
