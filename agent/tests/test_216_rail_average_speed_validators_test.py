"""
Offline unit tests for Tier 5: Derived composed-unit ratio (rail journey average speed) (test 216) -- free.
...
statement-leak test).
"""
from agent.app.idea_tests import test_216_tier5_rail_average_speed as t
...
_FULL_SINGLE = (
    "Tōkaidō Shinkansen (Japan): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
    "Tōkaidō Shinkansen (Japan): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
...
def test_deliverables_list_primary_slot_drives_keystone():
    r = {
        "deliverables": [
            ...
            "Tōkaidō Shinkansen (Japan): " + _fmt(_A_VAL) + "; Tōkaidō Shinkansen (Japan): " + _fmt(_B_VAL) + ".",
...
def test_wrong_decoy_value_gates_credit_bearing_checks_to_zero():
    # The engineered near-miss decoy (dropping the 21 minutes and dividing by 2 hours flat = 257.7) reported instead of the
    ...
    wrong = (
        "Tōkaidō Shinkansen (Japan): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Tōkaidō Shinkansen (Japan): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "Computed value = " + _fmt(257.7) + " " + t.DERIVED_UNIT + " (a miscalculation)."
    )
    r = _r(wrong)
