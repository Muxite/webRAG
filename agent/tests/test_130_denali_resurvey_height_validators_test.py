...
def test_ungrounded_correct_value_scores_near_zero():
    ...
    overall = sum(v["score"] for v in [
        t.validate_keystone_elevation(r, ungrounded),
...
def test_compiled_plan_does_not_hand_the_reconcile_leaf_the_answer_url():
    ...
    injects its content as evidence, which -- for this task -- IS the authoritative Denali page
    that resolves the keystone. That silently grounds the reconcile step regardless of whether
    the upstream 'older'/'resurvey' visit leaves ever found the page themselves, defeating the
    'few reads, not breadth' design the task statement asks for. The upstream leaves must remain
    the only source of genuine navigation."""
    plan = t.get_compiled_plan()
