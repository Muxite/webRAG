"""
Offline unit tests for Tier 5: Derived dimensionless ratio (lake surface-area multiple) (test 217) -- free.
...
statement-leak test).
"""
from agent.app.idea_tests import test_217_tier5_lake_area_ratio as t
...
_FULL_SINGLE = (
    "Lake Titicaca (Peru/Bolivia): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
    "Lake Tahoe (California/Nevada, USA): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
...
def test_deliverables_list_primary_slot_drives_keystone():
    r = {
        "deliverables": [
            ...
            "Lake Titicaca (Peru/Bolivia): " + _fmt(_A_VAL) + "; Lake Tahoe (California/Nevada, USA): " + _fmt(_B_VAL) + ".",
...
def test_wrong_decoy_value_gates_credit_bearing_checks_to_zero():
    # The engineered near-miss decoy (inverted ratio (Tahoe / Titicaca) = 0.0585) reported instead of the
    ...
    wrong = (
        "Lake Titicaca (Peru/Bolivia): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Lake Tahoe (California/Nevada, USA): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "Computed value = " + _fmt(0.0585) + " " + t.DERIVED_UNIT + " (a miscalculation)."
    )
    r = _r(wrong)
