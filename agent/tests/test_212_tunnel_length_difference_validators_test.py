"""
Offline unit tests for Tier 5: Derived difference over same-unit operands (rail tunnel length gap) (test 212) -- free.
...
statement-leak test).
"""
from agent.app.idea_tests import test_212_tier5_tunnel_length_difference as t
...
_FULL_SINGLE = (
    "Gotthard Base Tunnel (Switzerland): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
    "Seikan Tunnel (Japan): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
...
def test_deliverables_list_primary_slot_drives_keystone():
    r = {
        "deliverables": [
            ...
            "Gotthard Base Tunnel (Switzerland): " + _fmt(_A_VAL) + "; Seikan Tunnel (Japan): " + _fmt(_B_VAL) + ".",
...
def test_wrong_decoy_value_gates_credit_bearing_checks_to_zero():
    # The engineered near-miss decoy (using the Gotthard COMBINED shaft/passage total (151.840 km) by mistake = 97.99) reported instead of the
    ...
    wrong = (
        "Gotthard Base Tunnel (Switzerland): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Seikan Tunnel (Japan): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "Computed value = " + _fmt(97.99) + " " + t.DERIVED_UNIT + " (a miscalculation)."
    )
    r = _r(wrong)
