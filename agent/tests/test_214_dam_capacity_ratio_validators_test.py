"""
Offline unit tests for Tier 5: Derived dimensionless ratio (hydropower installed-capacity multiple) (test 214) -- free.
...
statement-leak test).
"""
from agent.app.idea_tests import test_214_tier5_dam_capacity_ratio as t
...
_FULL_SINGLE = (
    "Xiluodu Dam (China): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
    "Longtan Dam (China): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
...
def test_deliverables_list_primary_slot_drives_keystone():
    r = {
        "deliverables": [
            ...
            "Xiluodu Dam (China): " + _fmt(_A_VAL) + "; Longtan Dam (China): " + _fmt(_B_VAL) + ".",
...
def test_wrong_decoy_value_gates_credit_bearing_checks_to_zero():
    # The engineered near-miss decoy (inverted ratio (Longtan / Xiluodu) = 0.4636) reported instead of the
    ...
    wrong = (
        "Xiluodu Dam (China): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Longtan Dam (China): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "Computed value = " + _fmt(0.4636) + " " + t.DERIVED_UNIT + " (a miscalculation)."
    )
    r = _r(wrong)
