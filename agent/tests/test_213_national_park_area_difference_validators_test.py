"""
Offline unit tests for Tier 5: Derived difference over same-unit operands (national park area gap) (test 213) -- free.
...
statement-leak test).
"""
from agent.app.idea_tests import test_213_tier5_national_park_area_difference as t
...
_FULL_SINGLE = (
    "Wood Buffalo National Park (Canada): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
    "Kruger National Park (South Africa): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
...
def test_deliverables_list_primary_slot_drives_keystone():
    r = {
        "deliverables": [
            ...
            "Wood Buffalo National Park (Canada): " + _fmt(_A_VAL) + "; Kruger National Park (South Africa): " + _fmt(_B_VAL) + ".",
...
def test_wrong_decoy_value_gates_credit_bearing_checks_to_zero():
    # The engineered near-miss decoy (sum instead of difference = 64364.0) reported instead of the
    ...
    wrong = (
        "Wood Buffalo National Park (Canada): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Kruger National Park (South Africa): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "Computed value = " + _fmt(64364.0) + " " + t.DERIVED_UNIT + " (a miscalculation)."
    )
    r = _r(wrong)
