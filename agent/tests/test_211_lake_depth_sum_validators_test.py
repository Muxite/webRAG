"""
Offline unit tests for Tier 5: Derived sum over same-unit operands (combined lake max depth) (test 211) -- free.

Covers the keystone gate (grounded computed sum), in single- and multi-line
...
statement-leak test).
"""
from agent.app.idea_tests import test_211_tier5_lake_depth_sum as t
...
_FULL_SINGLE = (
    "Lake Baikal (Siberia, Russia): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
    "Lake Tanganyika (East Africa): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
    "Computed sum = " + _fmt(t.DERIVED) + " " + t.DERIVED_UNIT + "."
)
...
def test_deliverables_list_primary_slot_drives_keystone():
    r = {
        "deliverables": [
            ...
            "Lake Baikal (Siberia, Russia): " + _fmt(_A_VAL) + "; Lake Tanganyika (East Africa): " + _fmt(_B_VAL) + ".",
...
def test_wrong_decoy_value_gates_credit_bearing_checks_to_zero():
    # The engineered near-miss decoy (difference instead of sum = 172.0) reported instead of the
    ...
    wrong = (
        "Lake Baikal (Siberia, Russia): " + _fmt(_A_VAL) + " (" + t.OP_A["url"] + "). "
        "Lake Tanganyika (East Africa): " + _fmt(_B_VAL) + " (" + t.OP_B["url"] + "). "
        "Computed value = " + _fmt(172.0) + " " + t.DERIVED_UNIT + " (a miscalculation)."
    )
    r = _r(wrong)
