...
- Branch `compiled-scaffold-dag`. `git log --oneline -8 -- services/agent/app/testing/execution_compiled.py`
  ...
  everything below. `git diff --stat HEAD -- services/agent/app/testing/execution_compiled.py` was
...
6. **Full offline suite** was green at 2307 passed / 18 skipped when this was written — re-run before
   trusting that's still current (`PYTHONPATH=services ./.venv/bin/python3 -m pytest services/agent/tests
   services/agent/app/idea_tests -q`).
