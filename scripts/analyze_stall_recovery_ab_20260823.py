#!/usr/bin/env python3
"""Analyze the 2026-08-23 stall_recovery_gate A/B: langgraph_react with the gate ON vs OFF
(`stall_ab_20260823_stallon` / `stall_ab_20260823_stalloff`), both with candidate_coverage_gate
and context_trim at their now-default-ON settings, good_adaptive arm, qwen2.5:7b, the same 6
breadth-shaped tasks (152-157), 2 reps/task = 12 cells/condition (24 total).

Neither `langgraph_react` nor `sequential_react` had ANY dead-end detection before this session
(confirmed by a capability survey) -- `stall_recovery_gate` is the first attempt at one: after N
consecutive non-progress tool results, it nudges the model toward a different approach. Unlike
the coverage gate and context_trim (both deterministic, "you missed something named"/"less
noise" mechanisms), this changes strategy on a judgment call, so its effect on score is
genuinely uncertain -- hence a live A/B before it becomes the default too.

Same paired-on-(task_id, rep) methodology as the other 2026-08-23 A/B scripts.

Usage: PYTHONPATH=.:services:agent ./.venv/bin/python \
    scripts/analyze_stall_recovery_ab_20260823.py
"""
import glob
import json
import re
...
RUN_IDS = {
    "stall_on": "stall_ab_20260823_stallon",
    "stall_off": "stall_ab_20260823_stalloff",
}
...
def main():
    on_rows, n_on_files = load_condition("stall_on", RUN_IDS["stall_on"])
    off_rows, n_off_files = load_condition("stall_off", RUN_IDS["stall_off"])

    print(f"stall_on:  {n_on_files} cell files loaded "
          f"({len(on_rows)}/{PLANNED_CELLS_PER_CONDITION} planned parsed OK)")
    print(f"stall_off: {n_off_files} cell files loaded "
    ...
    print(f"  stall_on-only (dropped, no match): {len(on_only)}")
    print(f"  stall_off-only (dropped, no match): {len(off_only)}")
    print()
    ...
    print("=== stall_recovery_gate ON vs OFF, both with coverage_gate+context_trim at their "
          "new defaults (paired, langgraph_react, good_adaptive, qwen2.5:7b, 6-task breadth "
          "pilot, 2 reps) ===")
    ...
    if stats:
        ...
        print(f"  SCORE  mean delta (stall_on - stall_off): {stats['mean']:+.3f} "
              f"(sd={sd_str}, n={stats['n']}, t={t_str})")
        print(f"  W/T/L: {wtl[0]}/{wtl[1]}/{wtl[2]}")
        print(f"  stall_on  mean score: {statistics.fmean(on_scores):.3f}")
        print(f"  stall_off mean score: {statistics.fmean(off_scores):.3f}")
    else:
    ...
    if vstats:
        ...
        print(f"  VISITS  mean delta (stall_on - stall_off): {vstats['mean']:+.2f} "
    ...
    for t in TASKS:
        ...
        print(f"    stall_on  scores={[round(v,2) for v in on_list]} mean={on_mean:.3f} "
              f"visits={per_task[t]['on_v']}")
        print(f"    stall_off scores={[round(v,2) for v in off_list]} mean={off_mean:.3f} "
