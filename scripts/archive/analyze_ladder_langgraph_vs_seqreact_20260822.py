#!/usr/bin/env python3
"""Analyze the 2026-08-22 langgraph_react vs sequential_react partial live A/B
(`ladder_langgraph_20260822` / `ladder_seqreact_20260822`, core24 task set, good_adaptive
arm, qwen2.5:7b, planned 72 cells/engine, hard-killed mid-run: 41/72 langgraph_react cells
and 22/72 sequential_react cells landed).

This was meant to re-measure the token/score gap between the native graph engine (run
through the "langgraph_react" execution-variant label -- this is this repo's own graph
engine, NOT LangGraph's off-the-shelf create_react_agent) and sequential_react, post the
merge-compaction fix landed earlier this session (see
project_merge_compaction_bug / docs/handoffs referenced in MEMORY.md).

Pairs strictly on (task_id, rep) present in BOTH engine result sets -- task IDs where only
one engine finished are dropped, not compared. With ~41 and ~22 raw cells respectively, the
paired-complete n is expected to be well under the 72 originally planned per engine; treat
all output here as UNDERPOWERED / directional only, not a confirmatory result.

Usage: PYTHONPATH=.:services:agent ./.venv/bin/python \
    scripts/analyze_ladder_langgraph_vs_seqreact_20260822.py
"""
import glob
import json
import re
...
RUN_IDS = {
    "langgraph_react": "ladder_langgraph_20260822",
    "sequential_react": "ladder_seqreact_20260822",
}
...
def main():
    ...
    print(f"  *** UNDERPOWERED: {len(score_deltas)} paired cells vs "
          f"{PLANNED_CELLS_PER_ENGINE} planned per engine ({PLANNED_CELLS_PER_ENGINE} x 2 = "
          f"144 total cells originally planned for this comparison) ***")
    print()
