#!/usr/bin/env bash
# Live-validate the final new batch (072-076). nano, parametric vs graph_compiled,
# R=2, record fixtures, tier 0, hard ceiling. Then gate_report.
set -u
cd /home/muk/projects/webRAG
PY=./.venv/bin/python
export PYTHONPATH=.:services:agent
OUT=agent/idea_test_results
LOG="$OUT/_validate_new2/driver.log"
mkdir -p "$OUT/_validate_new2"
...
export IDEA_TEST_IDS=072,073,074,075,076
...
export IDEA_TEST_RUN_ID=validate_new2

echo "[$(date -u +%FT%TZ)] ==== VALIDATE-NEW2 START (072-076) ====" | tee -a "$LOG"
"$PY" -m agent.app.idea_test_runner >>"$LOG" 2>&1
echo "[$(date -u +%FT%TZ)] ==== runner exit $? ====" | tee -a "$LOG"
echo "---- result files by test ----" | tee -a "$LOG"
for t in 072 073 074 075 076; do
  n=$(ls "$OUT"/validate_new2_${t}_*.json 2>/dev/null | grep -v summary | wc -l)
  echo "  test $t: $n result file(s)" | tee -a "$LOG"
done
echo "---- gate_report (want parametric LOW <=0.35, graph_compiled HIGH >=0.80) ----" | tee -a "$LOG"
"$PY" scripts/gate_report.py --run-id validate_new2 2>>"$LOG" | tee -a "$LOG"
echo "[$(date -u +%FT%TZ)] ==== VALIDATE-NEW2 DONE ====" | tee -a "$LOG"
