#!/usr/bin/env python3
"""Analyze the 2026-08-23 REDUCED-SCOPE langgraph_react vs sequential_react local A/B
(`ladder_reduced_20260823_graph` / `ladder_reduced_20260823_seqreact`), an 8-task subset of
core24, good_adaptive arm, qwen2.5:7b, 3 reps/task = 24 cells/engine (48 total).

This re-run exists because the prior full 144-cell re-run
(`ladder_langgraph_full_20260823`/`ladder_seqreact_full_20260823`) and the Cycle 2 diagnostic
run (`diag_parallel_dep_20260823`) were discovered to have run against a DEAD Serper search key
(403 Unauthorized) -- see the "2026-08-23 addendum" sections in
docs/handoffs/GRAPH_VS_SEQREACT_GAP_INVESTIGATION_2026-08-22.md, now marked RETRACTED. This
script mirrors analyze_ladder_langgraph_vs_seqreact_full_20260823.py's methodology exactly
(paired on (task_id, rep)), pointed at the new run-ids and the smaller 8-task set.

Usage: PYTHONPATH=.:services:agent ./.venv/bin/python \
    scripts/analyze_ladder_reduced_20260823.py
"""
import glob
import json
import re
...
RUN_IDS = {
    "langgraph_react": "ladder_reduced_20260823_graph",
    "sequential_react": "ladder_reduced_20260823_seqreact",
}
TASKS = ["122", "126", "128", "132", "134", "137", "141", "144"]
REPS = 3
PLANNED_CELLS_PER_ENGINE = len(TASKS) * REPS  # 8 x 3 = 24
...
def main():
    ...
    print(f"  planned: {len(score_deltas)} paired cells vs "
