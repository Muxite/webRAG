...
#
# Survives terminal close (launch via: setsid nohup scripts/barrage_continue_batch5.sh &).
...
DRIVER_DIR="$RESULTS_DIR/_driver_${RUN_ID}_batch5"
...
# barrage24b spent $29.47 after the 065/069 bugfixes; cap = that + ~$15 for batch 5 (3 reworked + 5 brand-new tests).
GLOBAL_USD_CAP=45.00
PER_INV_CEILING=3.00      # backstop inside a single runner invocation (runaway guard)

NEW_TESTS="073 080 081 090 091 092 093 094"       # full matrix — confirmed discriminators, never full-matrix tested
...
write_status(){
  ...
  local spent; spent="$(cum_spend)"
  {
    ...
    echo
    echo "- run_id: \`$RUN_ID\` (batch5 driver)"
    echo "- updated: $now"
...
finish(){ local why="$1"; log "BARRAGE BATCH5 END ($why) | cumulative=\$$(cum_spend)"; CURRENT_STAGE="ended: $why"; aggregate; rm -f "$LOCK"; }

# ===========================================================================
log "==== BARRAGE BATCH5 START ===="
...
CURRENT_STAGE="Batch 5: full matrix on 073,080,081,090,091,092,093,094 (rework + new)"
write_status
