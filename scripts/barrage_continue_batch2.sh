...
#
# Survives terminal close (launch via: setsid nohup scripts/barrage_continue_batch2.sh &).
...
NEW_TESTS="064 065 066 068 069"       # full matrix — confirmed discriminators, never full-matrix tested
...
write_status(){
  ...
  local spent; spent="$(cum_spend)"
  {
    ...
    echo
    echo "- run_id: \`$RUN_ID\` (batch2 driver)"
    echo "- updated: $now"
...
finish(){ local why="$1"; log "BARRAGE BATCH2 END ($why) | cumulative=\$$(cum_spend)"; CURRENT_STAGE="ended: $why"; aggregate; rm -f "$LOCK"; }

# ===========================================================================
log "==== BARRAGE BATCH2 START ===="
...
CURRENT_STAGE="Batch 2: full matrix on 064,065,066,068,069"
write_status
