...
#
# Survives terminal close (launch via: setsid nohup scripts/barrage_continue_batch4.sh &).
...
DRIVER_DIR="$RESULTS_DIR/_driver_${RUN_ID}_batch4"
...
# barrage24b spent $21.80 through batch 3; cap = that + ~$22 approved for batch 4 (final tier-5 test batch).
GLOBAL_USD_CAP=44.00
PER_INV_CEILING=3.00      # backstop inside a single runner invocation (runaway guard)

NEW_TESTS="079 082 085 086 087 088 089"       # full matrix — confirmed discriminators, never full-matrix tested
...
write_status(){
  ...
  local spent; spent="$(cum_spend)"
  {
    ...
    echo
    echo "- run_id: \`$RUN_ID\` (batch4 driver)"
    echo "- updated: $now"
...
finish(){ local why="$1"; log "BARRAGE BATCH4 END ($why) | cumulative=\$$(cum_spend)"; CURRENT_STAGE="ended: $why"; aggregate; rm -f "$LOCK"; }

# ===========================================================================
log "==== BARRAGE BATCH4 START ===="
...
CURRENT_STAGE="Batch 4: full matrix on 079,082,085,086,087,088,089"
write_status
